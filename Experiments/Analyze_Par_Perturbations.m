...
load('Ensemble_Simulation_Behavioral_Parameters_Perturbation_Analysis_n1000ens.mat');
ens_initial_states = load('Ensemble_Simulation_Initial_Conditions_Perturbation_Analysis_n1000ens_BalancedVariance_No_alpha_beta_correction.mat');
...
x1_hat_fs = nan(1000,size(pertXu,3));
x2_hat_fs = nan(1000,size(pertXu,3));
x3_hat_fs = nan(1000,size(pertXu,3));
x4_hat_fs = nan(1000,size(pertXu,3));
x5_hat_fs = nan(1000,size(pertXu,3));

for t = 1:size(pertXu,3)-1
    totVar_pertUk(t) = trace(nancov(reshape(pertXu(:,:,t),1000,5)));
...
%     x1_hat_fs(:,t) = mean_xi(1,t)+reshape(pertXu(1,:,t),1000,1);
%     x2_hat_fs(:,t) = mean_xi(2,t)+reshape(pertXu(2,:,t),1000,1);
%     x3_hat_fs(:,t) = mean_xi(3,t)+reshape(pertXu(3,:,t),1000,1);
%     x4_hat_fs(:,t) = mean_xi(4,t)+reshape(pertXu(4,:,t),1000,1);
%     x5_hat_fs(:,t) = mean_xi(5,t)+reshape(pertXu(5,:,t),1000,1);
    
    %Option 2: Mean of the ensemble of initial states to run model forward
    x1_hat_fs(:,t) = ens_initial_states.mean_prediction.SM(t)+reshape(pertXu(1,:,t),1000,1);
    x2_hat_fs(:,t) = ens_initial_states.mean_prediction.ISU(t)+reshape(pertXu(2,:,t),1000,1);
    x3_hat_fs(:,t) = ens_initial_states.mean_prediction.ISO(1,t)+reshape(pertXu(3,:,t),1000,1);
    x4_hat_fs(:,t) = ens_initial_states.mean_prediction.ISO(2,t)+reshape(pertXu(4,:,t),1000,1);
    x5_hat_fs(:,t) = ens_initial_states.mean_prediction.ISO(3,t)+reshape(pertXu(5,:,t),1000,1);
