function [cSM, aET, cERI, cERO, infiltration] = ccrest(stepHours, precipIn, petIn, parameters, states)
...
% M3 = I
infiltration = ((parameters.PWM - states.SM).*approx_f3 + parameters.PWM.*((1-it./im).^(1+parameters.PB)-(1-((it+aET)./im)).^(1+parameters.PB)).*(1-approx_f3)).*approx_f4.*approx_f1;
